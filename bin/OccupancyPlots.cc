...
int OccupancyPlots(std::string const DataFileName, uint32_t StartTime, uint32_t EndTime)
{
  ...
  for (int ientry = 0; Event.GetNextEvent() >= 0; ++ientry)
  {
    ...
    for (size_t ip = 0; ip != Event.NTelescopes(); ++ip)
    {
      ...
      if (phit == 0x1)
        hCoincidenceMap[Tele->Channel()]->Fill(0); // only first plane hit
      if (phit == 0x2)
        hCoincidenceMap[Tele->Channel()]->Fill(1); // only 2nd plane hit
      if (phit == 0x4)
        hCoincidenceMap[Tele->Channel()]->Fill(2); // only 3rd plane hit

      if (phit == 0x3)
        hCoincidenceMap[Tele->Channel()]->Fill(3); // Plane 0 and 1 in coincidence
      if (phit == 0x6)
        hCoincidenceMap[Tele->Channel()]->Fill(4); // Plane 1 and 2 in coincidence
      if (phit == 0x5)
        hCoincidenceMap[Tele->Channel()]->Fill(5); // Plane 0 and 2 in coincidence
      if (phit == 0x7)
        hCoincidenceMap[Tele->Channel()]->Fill(6); // All planes in coincidence
    }
  ...
  for (std::map<int, TH1F *>::iterator it = hCoincidenceMap.begin(); it != hCoincidenceMap.end(); ++it)
  {
    ...
    char *bin[7] = {(char *)"ROC0", (char *)"ROC1", (char *)"ROC2", (char *)"ROC0&1", (char *)"ROC1&2", (char *)"ROC0&2", (char *)"All ROC Hit"};
