...
std::vector<int> channel_no;
std::vector<int> roc_no;
std::vector<int> row;
std::vector<int> col;
std::vector<int> pulseHeight;
...
bool decodeDataWord(uint32_t unsigned word, int counts[], std::ostream &myfile)
{
  if (fullDebug)
    // std::cout << std::hex << "0x" << word << std::dec << ": ";
  if (word & 0xfffffff)
  {
    ...
    if (roc > 25)
    {
      ...
      else if (roc == 30)
      {
        ...
        if (fullDebug)
        {
          ...
          if (word & (1 << 8))
            std::cout << ": data overflow";
          if (word & (3 << 9))
            std::cout << ": FSM error " << (word & (3 << 9));
          if (word & (1 << 11))
            std::cout << ": invalid # of ROCs";
          if (word & (0xff))
          {
            std::cout << " from TBM:";
            ...
            if (tbmErr & 1)
              std::cout << " stack full";
            if (tbmErr & (1 << 1))
              std::cout << " precal trigger issued";
            if (tbmErr & (1 << 2))
              std::cout << " event counter cleared";
            if (tbmErr & (1 << 3))
              std::cout << " sync trigger";
            if (tbmErr & (1 << 4))
              std::cout << " sync trigger error";
            if (tbmErr & (1 << 5))
              std::cout << " ROC reset";
            if (tbmErr & (1 << 6))
              std::cout << " TBM reset";
            if (tbmErr & (1 << 7))
              std::cout << " no token pass";
          }
          std::cout << std::endl;
        }
      }
      else if (roc == 31)
      {
        counts[3]++;
        if (fullDebug)
          std::cout << "Event# Error: channel " << chan << " event " << (word & 0xff) << " " << std::endl;
      }
      return false;
    }
    else if (chan > 0 && chan < 37)
    {
      ...
      if (roc <= 2)
      {
        ...
        // myfile <<" Hit: " << chan << "-" << roc << "@" << mycol << "," << abs(convertPixel(rawPixel)) << std::endl;

        channel_no.push_back(chan);
        roc_no.push_back(roc);
        row.push_back(abs(convertPixel(rawPixel)));
        col.push_back(mycol);
        pulseHeight.push_back((int)(word & plsmsk));
        counts[0]++;
      }
      else
      {
...
int readEvent(std::ifstream &infile, std::ofstream &myfile)
{
      ...
      while (bheader)
      {
        ...
        else
        {
          ...
          if ((peekWord & 0xff000000) == 0xa0000000)
          {
            ...
            if (n2 != oldn2)
              decodeDataWord(n2, counts, myfile);
            nWords += 1;
          }
          else
          {
            ...
            if (n2 != oldn2)
              decodeDataWord(n2, counts, myfile);
            if (n1 != oldn1)
              decodeDataWord(n1, counts, myfile);
          }
        }
      } // reading event data
    }
  ...
  if (counts[0] > 0)
  {
    if (counts[0] != channel_no.size())
    {
      std::cout << "Lenght does not match" << std::endl;
      exit(1);
    }
    for (int i=0; i<counts[0]; i++)
    {
      ...
      //        << "Hits: " << counts[0] << ", "
      //        << "ch: " << ch << std::endl;
      myfile << event << "," << time << "," << bx + 1 << "," << counts[0] << "," << channel_no.at(i) << "," << roc_no.at(i) << "," << row.at(i) << "," << col.at(i) << "," << pulseHeight.at(i) << std::endl;
    }
    channel_no.clear();
  }
...
int main(const int argc, const char **argv)
{
  ...
  std::ofstream myfile;
  myfile.open("hits_test.csv");
  myfile << "event,timestamp,BCID,hits,ch,roc,row,col,pulseHeight\n";

  int nEvents = 0;
  while (!infile.eof())
  {
    readEvent(infile, myfile);
    nEvents++;
    // if (nEvents >= 10000)
    //   break;
  }
