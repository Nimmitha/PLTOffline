      ...
      while (bheader)
      {
        ...
        else
        {
          ...
          if ((peekWord & 0xff000000) == 0xa0000000)
          {
            ...
            if (n2 != oldn2)
              DecodeSpyDataFifo(n2, Hits, Errors, DesyncChannels);
          }
          else
          {
            ...
            if (n2 != oldn2)
              DecodeSpyDataFifo(n2, Hits, Errors, DesyncChannels);
            if (n1 != oldn1)
              DecodeSpyDataFifo(n1, Hits, Errors, DesyncChannels);
          }
        } // not a trailer
      }   // after header
    }
...
void PLTBinaryFileReader::ReadOnlinePixelMask(const std::string maskFileName, const PLTGainCal &gainCal)
{
  ...
  while (1)
  {
    ...
    if (ch == -1)
      continue; // conversion failed -- this is probably a scope in the mask file not actually functional
